...
class QLearningAgent(ReinforcementAgent):
    ...
    def getAction(self, state):
        ...
        if util.flipCoin(self.epsilon):
            exploration = random.choice(legalActions)
            return exploration
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.Q[(state, action)] = self.getQValue(state, action) + self.alpha * ((reward +
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        # if this variable > epsilon means that we need to stop learning because
        # we do not lean anything useful.
        dif = reward + \
            self.discount * \
        ...
        for i in self.featExtractor.getFeatures(state, action).keys():
            self.weights[i] = self.weights[i] + self.alpha * \
                dif * self.featExtractor.getFeatures(state, action)[i]
