...
class RegularizedLinearRegressor_Multi:
    ...
    def train(self, X, y, reg=1e-5, num_iters=100):
        ...
        theta_opt = scipy.optimize.fmin_bfgs(
            self.loss, theta, fprime=self.grad_loss, args=(X, y, reg), maxiter=num_iters)
...
class RegularizedLinearReg_SquaredLoss(RegularizedLinearRegressor_Multi):
    ...
    def loss(self, *args):
        ...
        #######################################################################
        J = np.square((np.dot(X, theta) - y)).sum() + \
            (reg * (np.square(theta[1:]).sum()))
        J = J / (2 * num_examples)
        #######################################################################
        #                           END OF YOUR CODE                              #
        #######################################################################
