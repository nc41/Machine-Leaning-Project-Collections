...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3 * 32 * 32, hidden_dim=100, num_classes=10,
        ...
        self.M = hidden_dim
        self.C = num_classes
        theta1 = weight_scale * np.random.randn(self.D, self.M)
        theta1_0 = np.zeros(self.M)
        theta2 = weight_scale * np.random.randn(self.M, self.C)
        theta2_0 = np.zeros(self.C)

        self.params.update({'theta1': theta1, 'theta1_0': theta1_0,
                            'theta2': theta2, 'theta2_0': theta2_0})
        #######################################################################
        #                             END OF YOUR CODE                             #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        w1, b1, w2, b2 = self.params['theta1'], self.params['theta1_0'], self.params['theta2'], self.params['theta2_0']
        X_new = X.reshape(X.shape[0], self.D)
        hidden, hidden_cache = affine_relu_forward(X_new, w1, b1)
        ...
        di, dw1, db1 = affine_relu_backward(dh, hidden_cache)
        dw1 += self.reg * w1

        grads.update({'theta1': dw1, 'theta1_0': db1,
                      'theta2': dw2, 'theta2_0': db2})
        #######################################################################
        #                             END OF YOUR CODE                             #
        #######################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        w = {'theta' + str(i + 1): weight_scale * np.random.randn(layers[i], layers[i + 1])
             for i in range(self.num_layers - 1)}
        b = {'theta' + str(i + 1) + '_0': np.zeros(layers[i + 1])
             for i in range(self.num_layers - 1)}
    ...
    def loss(self, X, y=None):
        ...
        if self.use_dropout:
            hdrop, hdrop_cache = dropout_forward(
                hidden['h0'], self.dropout_param)
            hidden['hdrop0'] = hdrop
            hidden['hdrop_cache0'] = hdrop_cache

        for i in range(self.num_layers - 1):
            idx = i + 1
            w = self.params['theta' + str(idx)]
            b = self.params['theta' + str(idx) + '_0']
            h = hidden['h' + str(idx - 1)]
            if self.use_dropout:
                h = hidden['hdrop' + str(i)]

            # last layer we use affine_forward without activation
            if idx == self.num_layers - 1:
                ...
                if self.use_dropout:
                    hdrop, hdrop_cache = dropout_forward(
                        hidden['h' + str(idx)], self.dropout_param)
                    hidden['hdrop' + str(idx)] = hdrop
                    hidden['hdrop_cache' + str(idx)] = hdrop_cache
        scores = hidden['h' + str(self.num_layers - 1)]
        #######################################################################
        #                             END OF YOUR CODE                             #
        #######################################################################
        ...
        for i in range(self.num_layers - 1)[::-1]:
            ...
            if idx == self.num_layers - 1:
                ...
                hidden['dh' + str(i)] = dh
                grads['theta' + str(idx)] = dw + self.reg * \
                    self.params['theta' + str(idx)]
                grads['theta' + str(idx) + '_0'] = db
            else:
                if self.use_dropout:
                    cache_hdrop = hidden['hdrop_cache' + str(idx)]
