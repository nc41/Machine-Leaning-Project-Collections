...
class LogisticRegressor:
    ...
    def loss(self, *args):
        ...
        #######################################################################
        J = -1 * (np.sum(np.log(utils.sigmoid(np.dot(theta, X.T))) * y +
                         (np.log(1.0 - utils.sigmoid(np.dot(theta, X.T))) * (1.0 - y)))) / m
        #######################################################################
        #                           END OF YOUR CODE                              #
        #######################################################################
        return J
    ...
    def predict(self, X):
        ...
        # TODO: 1 line of code expected                                           #
        #######################################################################
        y_pred = np.round(utils.sigmoid(np.dot(self.theta, X.T)))
        #######################################################################
        #                           END OF YOUR CODE                              #
        #######################################################################
        return y_pred
...
class RegLogisticRegressor:
    ...
    def loss(self, *args):
        ...
        J = -1 * (np.sum(np.log(utils.sigmoid(np.dot(theta, X.T))) * y +
                         (np.log(1.0 - utils.sigmoid(np.dot(theta, X.T))) * (1.0 - y)))) / m
        J = J + (reg / (2.0 * m)) * np.sum((theta[1:]**2))
        #######################################################################
        #                           END OF YOUR CODE                              #
        #######################################################################
        return J
    ...
    def grad_loss(self, *args):
        ...
        # TODO: 1 line of code expected                                          #
        #######################################################################
        grad = np.dot((utils.sigmoid(np.dot(theta, X.T)) - y),
                      X) / m + np.hstack((np.array((0)), theta[1:])) * reg / m
        #######################################################################
        #                           END OF YOUR CODE                              #
        #######################################################################
        return grad
